...
public class OuttakeIO_Real implements OuttakeIO {
  ...
  public OuttakeIO_Real() {
    ...
    pivotConfigs.Feedback.SensorToMechanismRatio = 1.0 / OuttakeConstants.kGearingPivot; //Sets default output to pivot rotations
    ...
    flywheelConfigs.Feedback.SensorToMechanismRatio = 1.0 / OuttakeConstants.kGearingFlywheel; //Sets default output to flywheel rotations
  ...
  public void updateInputs(OuttakeIOInputs inputs) {
    ...
    inputs.pivotMotorPosition = Units.rotationsToDegrees(pivotMotor.getPosition().getValueAsDouble()); //Degrees
    inputs.pivotMotorVelocity = Units.rotationsToDegrees(pivotMotor.getVelocity().getValueAsDouble()); //Degrees per second
  ...
  public void changeFlywheelSetpoint(double rpm) {
    leaderFlywheel.setControl(new VelocityVoltage(MathUtil.clamp(rpmSetpoint, OuttakeConstants.kMinRpm, OuttakeConstants.kMaxRpm) / 60)); //RPM to Native Rotations per second
  }
  ...
  public void changePivotSetpoint(double angleDegrees) {
    ...
    pivotMotor.setControl(new MotionMagicVoltage(Units.degreesToRotations(rawAngleSetpoint))); //Degrees to Native Rotations
  }
}
